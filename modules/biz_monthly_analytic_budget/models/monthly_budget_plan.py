...
class MonthlyBudgetPlan(models.Model):
    ...
    def _sync_existing_po_reservations(self):
        """Backfill/update reservation commitments for confirmed POs that already exist."""
        BudgetLine = self.env['monthly.budget.line']
        Commitment = self.env['budget.commitment'].sudo()
        PurchaseOrder = self.env['purchase.order'].sudo()
        engine = self.env['budget.engine']
    ...
    def action_confirm(self):
        ...
        # Refresh the current plan snapshot and dashboard MV after backfill.
    ...
    def action_recompute_budget(self):
        ...
        Existing PR and confirmed PO reservations are also synced into
        budget.commitment so live documents have matching audit records.
        """
        self.ensure_one()
